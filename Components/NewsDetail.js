...
export default class NewsDetail extends React.Component {
  ...
  componentDidMount() {
     ...
     this.setState({
       DetailNews:    this.props.navigation.state.params.ContentNews,
       isLoading: false
     });
  ...
  <View style={styles.view_container}>
      <ScrollView style={styles.scrollview_container}>
        <Image
          style={ styles.image}
           source={{uri: 'https://www.betroulette.net/benarous/public/upload/news/'+this.state.DetailNews.image }}
        />

        <View style={styles.main_container} >
              <Text  style={styles.title_text} >{this.state.DetailNews.titre}</Text>
              <Text  style={styles.date_text} > <Material name='clock-outline' size={10} /> { this.state.DetailNews.dates } </Text>
              <Text  style={styles.description_text }> {this.state.DetailNews.description}</Text>
          </View>
